...
import { TEST_RESOURCE_BALANCE } from "@/features/users/roles";
...
export async function loadGymitionAppState() {
  ...
  const state: GymitionState = {
    user: {
      ...
      coins: appUser.role === "tester" ? TEST_RESOURCE_BALANCE : appUser.coins,
      xp: appUser.role === "tester" ? TEST_RESOURCE_BALANCE : appUser.xp,
